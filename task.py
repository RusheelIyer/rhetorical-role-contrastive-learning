...
class Task:
    def __init__(self,
                 ...
                 num_fols,
                 max_docs=-1,
                 dev_metric="weighted-f1",
                 portion_training_data=1.0,
        ...
        self.data_dir = os.path.join(
            data_folder,
            task_name if task_folder_name is None else task_folder_name)
    ...
    def _get_batches(self, examples):
        ds_builder = BatchCreator(examples,
                                  tokenizer=None,
                                  labels=self.labels,
    ...
    def _load_full_set(self, file_suffix='legalbert'):
        ...
        full_examples = DocumentsDataset(os.path.join(
            self.data_dir, f"full_{file_suffix}.txt"),
    ...
    def _load_train_dev_test_examples(self, file_suffix='legalbert') -> Fold:
        ...
        train_examples = DocumentsDataset(os.path.join(
            self.data_dir, f"train_{file_suffix}.txt"),
                                          max_docs=self.max_docs)
        dev_examples = DocumentsDataset(os.path.join(self.data_dir,
                                                     f"dev_{file_suffix}.txt"),
                                        max_docs=self.max_docs)
        test_examples = DocumentsDataset(os.path.join(
            self.data_dir, f"test_{file_suffix}.txt"),
                                         max_docs=self.max_docs)
    ...
    def get_folds_examples(self, file_suffix='legalbert'):
        ...
        if self.num_folds == 1:
            self.folds_examples = self._load_train_dev_test_examples(
                file_suffix=file_suffix)
        else:
            full_examples = np.array(
                self._load_full_set(file_suffix=file_suffix))
            ...
            for train_index, test_index in kf.split(full_examples):
                ...
                train, dev = train_test_split(train_and_dev,
                                              test_size=1.0 / self.num_folds,
                                              shuffle=False)
